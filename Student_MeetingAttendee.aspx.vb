...
Public Class Student_MeetingAttendee
...
#Region "loaddata"
    ...
    Protected Sub ddlProject_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlProject.SelectedIndexChanged
        ...
        SQL(0) = "  Select teamtitle, meetingId, meetingTopic, " _
                 & " e.stuId, concat(e.stuId, '-', e.stuName) as stuStr " _
                 & " from team a " _
                 ...
                 & " inner join student e on c.stuId = e.stuId  " _
                 & " where c.stuId = '" & Session("userId") & "' " _
                 & " And a.projId = " & ddlProject.SelectedValue & " " _
                 & " And (d.meetingStatus <> 'Canceled' or d.meetingStatus <> 'Completed') "
        DT = M1.GetDatatable(SQL(0))
        Try
